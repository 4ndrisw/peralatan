...
class Peralatan_mail_template
{
    ...
    private function get_language($email, $template = null, $params = [])
    {
        ...
        if ($rel_type != 'peralatan' && $rel_type != 'lead') {
        ...
        } elseif ($rel_type == 'peralatan') {
            $this->ci->db->select('rel_type, clientid');
            $this->ci->db->where('id', $this->get_clientid());
            $peralatan = $this->ci->db->get(db_prefix() . 'peralatan')->row();
            if ($peralatan && $peralatan->rel_type == 'lead') {
                $this->ci->db->select('default_language')
                ->where('id', $peralatan->clientid);

                $lead = $this->ci->db->get(db_prefix() . 'leads')->row();
            } elseif ($peralatan && $peralatan->rel_type == 'customer') {
                $customerDefault = get_client_default_language($peralatan->clientid);
    ...
    {
        include_once(peralatan_get_mail_template_path($className, $params));
